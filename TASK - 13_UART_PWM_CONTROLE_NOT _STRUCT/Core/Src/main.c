		...
		if(pwm_flag)
		{
			...
			if(pwm_flag)
			{
				...
				if(u2Data[0] == '@')
				{
					...
					while(u2Data[count0] != ':')
					{
						P_PWM.U_Freq = (P_PWM.U_Freq * 10) + (u2Data[count0] - '0');
						count0++;
					}
					count0++;
					P_PWM.U_Duty = 0;
					while(u2Data[count0] != '\0')
					{
						P_PWM.U_Duty = (P_PWM.U_Duty * 10) + (u2Data[count0] - '0');
						count0++;
					}
					count0 = 0;
				}
			...
			if(!(P_PWM.U_Freq) || (P_PWM.U_Freq>50000))
			{
			...
			P_PWM.R_Freq = PWM_MAX_FREQ / P_PWM.U_Freq;		// setect ACC value 
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if(u2Data[0] == '$' || (pwm_count == 0 && RxData == '$'))
	{
		u2Data[pwm_count++] = RxData;
		if(RxData == '\n')
		{
			//HAL_UART_Transmit(&huart2,(uint8_t *)u2Data,pwm_count, 100);
			u2Data[--pwm_count] = '\0';
			//HAL_UART_Transmit(&huart2,(uint8_t *)u2Data,pwm_count, 100);
			u2Data[0] =  '@';
			pwm_count =RxData =  0;
