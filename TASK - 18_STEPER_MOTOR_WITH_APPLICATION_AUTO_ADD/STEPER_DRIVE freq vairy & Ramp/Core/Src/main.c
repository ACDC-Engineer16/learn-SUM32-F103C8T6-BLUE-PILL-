...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define FORWD_COMM	"forwd"
#define REV_COMM		"backk"
#define FREQ_COMM		"freqq"
#define DUTY_COMM		"dutyy"
#define START_COMM	"start"
#define STOP_COMM		"stopp"
...
uint32_t targetFreq = 1000;
uint32_t requestedFreq = 1000;
...
void TIM2_Calculate(uint32_t freq, uint32_t *psc, uint32_t *arr)
{
...
void TIM2_Update(void)
{
    ...
    TIM2_Calculate(pwmFrequency, &psc, &arr);

    TIM1->PSC  = psc;
    TIM1->ARR  = arr;

    ccr = ((arr + 1) * pwmDuty) / 100;
...
Parser_Status_t Parse_Command(char *rx)
{
    ...
    if(String_Compare(command, FREQ_COMM))
    {
        //Motor_SetFrequency(value);
				targetFreq = value;
				requestedFreq  = value;
    }

    else if(String_Compare(command, DUTY_COMM))
    {
        Motor_SetDuty(value);
    }

    else if(String_Compare(command, FORWD_COMM))
    {
        if(value)
        {
					targetDir = 1;
    ...
    else if(String_Compare(command, REV_COMM))
    {
        if(value)
        {
					targetDir = 0;
		...
		else if(String_Compare(command, STOP_COMM))
    {
        if(value)
        {
            targetFreq = 0;
		...
		else if(String_Compare(command, START_COMM))
    {
        if(value)
        {
						targetFreq = requestedFreq;
						Motor_Start();
        }
    }
...
if(commandReady)
{
    ...
    switch(status)
    {
        ...
        case PARSER_ERROR:
            Debug_Print("ERR FRAME\r\n");
            break;

        case PARSER_INVALID_VALUE:
            Debug_Print("ERR VALUE\r\n");
            break;
