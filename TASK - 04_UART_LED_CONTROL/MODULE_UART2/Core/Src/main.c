...
/* USER CODE BEGIN PV */
uint8_t rxByte = 0;   /* ADD THIS LINE HERE */
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    HAL_UART_Transmit(&huart2,(uint8_t*)"Type 1=LED ON, 0=LED OFF: \n", 27, 100);

    

    /* Wait for 1 byte from user */
		L1:

    HAL_UART_Receive(&huart2, &rxByte, 1, HAL_MAX_DELAY);

    if (rxByte == '1')  /* user typed '1' */
